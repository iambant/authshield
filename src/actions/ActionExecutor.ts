...
export class ActionExecutor {
  ...
  public async execute(incident: Incident): Promise<void> {
    ...
    if (incident.action === "challenge_login") {
      const username = incident.evidence.username;
      if (typeof username === "string" && username.length > 0) {
        await this.store.setUserChallenge(username, this.options.challengeDurationMs);
      }
    }
  }
}
