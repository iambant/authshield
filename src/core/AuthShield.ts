...
import { RegistrationBurstDetector } from "../detectors/registrationBurst";
import { DisposableEmailAbuseDetector } from "../detectors/disposableEmailAbuse";
...
export interface AuthShieldConfig {
  ...
  registrationBurstWindowMs?: number;
  registrationBurstThreshold?: number;
  registrationBurstAction?: "challenge_login" | "block_ip";
  disposableEmailWindowMs?: number;
  disposableEmailThreshold?: number;
  disposableEmailAction?: "challenge_login" | "block_ip";
  registrationDisposableDomains?: string[];
  blockDisposableEmailOnRegistration?: boolean;
  onLimitEvent?: (
    event: LimitEvent,
    req: Request,
...
export interface DecisionPayload {
  flow: "pre_login_guard" | "pre_registration_guard" | "login" | "registration" | "token";
  event: Partial<LoginAttemptEvent & RegistrationAttemptEvent & TokenUsageEvent>;
  decision: MitigationDecision;
}

export type LimitEventType =
  | "rate_limit_ip"
  | "registration_rate_limit_ip"
  | "registration_disposable_email"
  | "rate_limit_ip_user"
  | "captcha_required"
  | "ip_blocked";
...
const defaultExtractors = {
  ...
  getEmail: (req: Request): string => String(req.body?.email ?? "").trim().toLowerCase(),
  getSessionId: (req: Request): string | null => {
...
export class AuthShield {
  ...
  private readonly registrationRateLimit: {
    windowMs: number;
    maxPerIp: number;
  };
  private readonly blockDisposableEmailOnRegistration: boolean;
  private readonly registrationDisposableDomainSet: Set<string>;
  ...
  private readonly getEmail: (req: Request) => string;
  ...
  public constructor(
    ...
    this.registrationRateLimit = {
      windowMs: config.registrationRateLimit?.windowMs ?? 60_000,
      maxPerIp: config.registrationRateLimit?.maxPerIp ?? 20,
    };
    this.blockDisposableEmailOnRegistration = config.blockDisposableEmailOnRegistration ?? false;
    this.registrationDisposableDomainSet = new Set(
      (config.registrationDisposableDomains ?? [...DISPOSABLE_EMAIL_DOMAINS]).map((domain) =>
        domain.toLowerCase(),
      ),
    );
    this.captcha = {
    ...
    this.getEmail = config.extractors?.getEmail ?? defaultExtractors.getEmail;
  ...
  public preRegistrationGuard(): RequestHandler {
    return async (req: Request, res: Response, next: NextFunction): Promise<void> => {
      const ip = this.getIp(req);
      ...
      const ipRateCount = await this.store.incrementWindowCounter(
        `register:ip:${ip}`,
        this.registrationRateLimit.windowMs,
        now,
      );

      if (ipRateCount > this.registrationRateLimit.maxPerIp) {
        const handled = await this.handleLimitEvent(
          req,
          res,
          {
            type: "registration_rate_limit_ip",
            ip,
            defaultAction: "challenge_login",
            defaultStatus: 429,
            defaultBody: {
              error: "Too many registration attempts from this IP",
              code: "AUTHSHIELD_REGISTRATION_RATE_LIMIT_IP",
            },
            retryAfterSec: Math.ceil(this.registrationRateLimit.windowMs / 1000),
          },
          "pre_registration_guard",
        );
        if (!handled) {
          next();
        }
      ...
      if (this.blockDisposableEmailOnRegistration) {
        const domain = this.extractEmailDomain(email);
        if (domain && this.registrationDisposableDomainSet.has(domain)) {
          const handled = await this.handleLimitEvent(
            req,
            res,
            {
              type: "registration_disposable_email",
              ip,
              email,
              defaultAction: "challenge_login",
              defaultStatus: 403,
              defaultBody: {
                error: "Disposable email is not allowed",
                code: "AUTHSHIELD_REGISTRATION_DISPOSABLE_EMAIL",
              },
            },
            "pre_registration_guard",
          );
  ...
  public postRegistrationReporter(): RequestHandler {
    return (req: Request, res: Response, next: NextFunction): void => {
      res.on("finish", async () => {
        const success = res.statusCode >= 200 && res.statusCode < 300;
        const event: RegistrationAttemptEvent = {
          ip: this.getIp(req),
          username: this.getUsername(req) || undefined,
          email: this.getEmail(req) || undefined,
          success,
          userAgent: req.get("user-agent") ?? undefined,
          deviceId: this.resolveDeviceId(req, res, false),
          timestamp: Date.now(),
        ...
        try {
          await this.reportRegistration(event);
        } catch (error) {
          console.error("AuthShield postRegistrationReporter error", error);
        }
      });
  ...
  public async reportRegistration(event: RegistrationAttemptEvent): Promise<MitigationDecision> {
    const incidents = await this.engine.evaluateRegistration(event);
    const decision = await this.handleIncidents(incidents);
    await this.emitDecision({ flow: "registration", event, decision });
    return decision;
  }
  ...
  private extractEmailDomain(email: string | undefined): string | null {
    if (!email) {
      return null;
    }
  ...
  private async handleLimitEvent(
    req: Request,
    res: Response,
    event: LimitEvent,
    flow: "pre_login_guard" | "pre_registration_guard" = "pre_login_guard",
  ): Promise<boolean> {
    ...
    if (handlerResult?.allow) {
      await this.emitDecision({
        flow,
        event: { ip: event.ip, username: event.username, email: event.email },
        decision: {
          allowed: true,
          action: "allow",
          incidents: [],
...
export function authShield(config: AuthShieldConfig = {}): AuthShield {
  ...
  const engine = new DetectionEngine(
    ...
    [
      new RegistrationBurstDetector({
        windowMs: config.registrationBurstWindowMs ?? 60_000,
        attemptsThreshold: config.registrationBurstThreshold ?? 12,
        action: config.registrationBurstAction ?? "challenge_login",
      }),
      new DisposableEmailAbuseDetector({
        windowMs: config.disposableEmailWindowMs ?? 10 * 60 * 1000,
        attemptsThreshold: config.disposableEmailThreshold ?? 3,
        action: config.disposableEmailAction ?? "challenge_login",
        disposableDomains: config.registrationDisposableDomains ?? [...DISPOSABLE_EMAIL_DOMAINS],
      }),
    ],
    store,
  );
